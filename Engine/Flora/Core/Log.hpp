...
/* Include fmt/ostr.h after spdlog.h */
...
public:
  static inline void Init() {
    ...
    logSinks.emplace_back(
        std::make_shared<spdlog::sinks::basic_file_sink_mt>("FloraEngine.log",
                                                            true));

    logSinks[0]->set_pattern("%^[%T] %n: %v%$");
    logSinks[1]->set_pattern("[%T] [%l] %n: %v");

    sCoreLogger = std::make_shared<spdlog::logger>("FLORA",
                                                   begin(logSinks),
                                                   end(logSinks));
...
#define FE_CORE_TRACE(...) \
  ::FloraEngine::Log::GetCoreLogger()->trace(__VA_ARGS__)
...
#define FE_CORE_ERROR(...) \
  ::FloraEngine::Log::GetCoreLogger()->error(__VA_ARGS__)
#define FE_CORE_CRITICAL(...) \
  ::FloraEngine::Log::GetCoreLogger()->critical(__VA_ARGS__)
...
#define FE_CRITICAL(...) \
  ::FloraEngine::Log::GetClientLogger()->critical(__VA_ARGS__)